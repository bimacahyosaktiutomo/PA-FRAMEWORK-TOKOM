...
class Cart:
    ...
    #     item_ids = self.cart.keys()
    #     items = Item.objects.filter(item_id__in=item_ids)  # Fetch items from the database
    #     items_dict = {str(item.item_id): item for item in items}  # Create a dictionary to map item_ids to items

    #     for item_id, cart_item in self.cart.items():
    #         if item_id in items_dict:
    #             cart_item['item'] = items_dict[item_id]  # Assign the actual Item object
    #             cart_item['price'] = Decimal(cart_item['price'])
    #             cart_item['total_price'] = cart_item['price'] * cart_item['quantity']
    #             yield cart_item
